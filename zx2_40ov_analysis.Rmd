...
hFind <- function (x) {
  zx2_40ov@assays$RNA@data@Dimnames[[1]][grep(x,zx2_40ov@assays$RNA@data@Dimnames[[1]],ignore.case = T)]
}

#function to obtain raw gene expression info
infoRGE <- function (x) {
#info Raw Gene Expression
  RawExp<-FetchData(object = zx2_40ov, vars = x, slot = "counts")
  NonZero <-apply(FetchData(object = zx2_40ov, vars = x, slot = "counts"),1, function(row) all(row !=0 ))
...
zx2_40ov.data <- Read10X(data.dir = "raw_data/filtered_feature_bc_matrix")

# Create Seurat object 
zx2_40ov <- CreateSeuratObject(counts = zx2_40ov.data, project = "zx2_40ov", min.cells = 20, min.features = 200)
...
zx2_40ov[["percent.mt"]] <- PercentageFeatureSet(zx2_40ov, pattern = "^mt-")
VlnPlot(zx2_40ov, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)

# Plot gene count, transcript count, and % mito genes
plot1 <- FeatureScatter(zx2_40ov, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(zx2_40ov, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
CombinePlots(plots = list(plot1, plot2))

# Filter cells
zx2_40ov <- subset(zx2_40ov, subset = nFeature_RNA > 200 & nFeature_RNA < 8000 & percent.mt < 15 & nCount_RNA < 75000)
...
# Normalize the data
zx2_40ov <- NormalizeData(object = zx2_40ov, normalization.method = "LogNormalize", scale.factor = 1e4)
...
zx2_40ov <- FindVariableFeatures(zx2_40ov, selection.method = "vst", nfeatures = 2000)

# ID top 10 variable genes
top10 <- head(VariableFeatures(zx2_40ov), 10)

# Plot variable genes
plot1 <- VariableFeaturePlot(zx2_40ov)
...
all.genes <- rownames(zx2_40ov)
zx2_40ov <- ScaleData(zx2_40ov, features = all.genes)

# Use ID'd 2000 Variable genes to run PCA analysis
zx2_40ov <- RunPCA(zx2_40ov, features = VariableFeatures(object = zx2_40ov))
...
DimHeatmap(zx2_40ov, dims = 1:30, cells = 500, balanced = TRUE)
...
zx2_40ov <- JackStraw(zx2_40ov, num.replicate = 100, dims = 50)
zx2_40ov <- ScoreJackStraw(zx2_40ov, dims = 1:50)
JackStrawPlot(zx2_40ov, dims = 1:50)
plan("sequential")
...
zx2_40ov <- FindNeighbors(zx2_40ov, dims = 1:30)
zx2_40ov <- FindClusters(zx2_40ov, resolution = 0.5)

# Visualize with UMAP
zx2_40ov <- RunUMAP(zx2_40ov, dims = 1:30)
DimPlot(zx2_40ov, reduction = "umap", pt.size = 1,label = T)
...
FeaturePlot(object = zx2_40ov, c("ddx4","gsdf","col1a1a","cyp11a2","fli1a","nkl.2","mpx","mpeg1.1","dcn","hemgn"), cols = c("grey90", "blue"), pt.size = 0.5)
...
```{r save_file(initial)}
# Save object
zx2_40ov_initial <- zx2_40ov
save(zx2_40ov_initial, file = "zx2_40ov_initial.robj") 
...
load(file = "zx2_40ov_initial_orig.robj")
zx2_40ov <- zx2_40ov_initial_orig
...
plan("multiprocess", workers = 4)
allmarker <- FindAllMarkers(zx2_40ov)
plan("sequential")
allmarker %>% group_by(cluster) %>% top_n(n = 20, wt = avg_logFC)

# Plot oocytes markers(zp3) and early germ cells(ddx4)
DimPlot(zx2_40ov, label = T)
FeaturePlot(zx2_40ov, features = "ddx4",label = T)
FeaturePlot(zx2_40ov, features = "zp3",label = T)

# Subset to remove oocyte and other germ cell from the data, so cells that are not expressing the oocyte genes are used. 
zx2_40ov_no_oo_gc <- subset(zx2_40ov, idents = c("3","13","18"), invert= T)


# Save the name labels
labs <- colnames(zx2_40ov_no_oo_gc@assays$RNA@counts)
...
# Selected zp and zar genes based on biological knowledge that they are highly expressed in the oocytes. Those genes are on the top of "inferNonExpressedGenes(scl)" list. Only need a couple of candidate genes for contamination fraction determination.
...
# Save Object
save(scl, file = "soupx-zx2_40ov.robj")
...
scl$channels$Channel1$channelName <- "zx2_40ov"
...
zx2_40ov <- CreateSeuratObject(scl$atoc, project = "zx2_40ov", min.cells = 20, min.features = 200)
...
zx2_40ov <- subset(zx2_40ov, subset = nFeature_RNA > 200 & nFeature_RNA < 8000 & percent.mt < 20 & nCount_RNA < 75000)
...
plan("multiprocess", workers = 4)
zx2_40ov <- JackStraw(zx2_40ov, num.replicate = 100, dims = 40)
zx2_40ov <- ScoreJackStraw(zx2_40ov, dims = 1:40)
plan("sequential")

JackStrawPlot(zx2_40ov, dims = 1:40)

```


```{r cluster_and_UMAP_soupx, fig.height=6, fig.width=10}
zx2_40ov <- FindNeighbors(zx2_40ov, dims = 1:30)
zx2_40ov <- FindClusters(zx2_40ov, resolution = 0.5)

zx2_40ov <- RunUMAP(zx2_40ov, dims = 1:30)
DimPlot(zx2_40ov, reduction = "umap", pt.size = 1)

```


```{r save_object_after_soupx_ambient_RNA_removal}
zx2_40ov_soupx <- zx2_40ov
save(zx2_40ov_soupx, file = "zx2_40ov_soupx.robj")
...
# Because the non-deterministic nature of the analysis, we will load the original file from the manuscript here. You can also load your own object that you have generated with the identical settings, which should be very similar to what we had.
load(file = "zx2_40ov_soupx_orig.robj") 
zx2_40ov <- zx2_40ov_soupx_orig
...
sweep.res.list.st <- paramSweep_v3(zx2_40ov, PCs = 1:30, sct = F)
...
annotation <- zx2_40ov@meta.data$seurat_clusters
homotypic.prop <- modelHomotypic(annotation)           
nExp_poi <- round(0.05*length(zx2_40ov@active.ident))  
...
df <- doubletFinder_v3(zx2_40ov, PCs = 1:30, pN = 0.25, pK = 0.005, nExp = nExp_poi.adj, reuse.pANN = FALSE, sct = F)
pull_id <- as.matrix(df@active.ident)
calss_id <- data.frame(row.names = rownames(pull_id), "SorD" =df@meta.data$DF.classifications_0.25_0.005_321)
...
zx2_40ov_clean <- subset(df, cells = id_singlet)
...
FeaturePlot(zx2_40ov_clean, features = "gsdf")
...
zx2_40ov <- zx2_40ov_clean
zx2_40ov <- SCTransform(object = zx2_40ov, verbose = F)
zx2_40ov <- RunPCA(zx2_40ov, features = VariableFeatures(object = zx2_40ov))
ElbowPlot(zx2_40ov, ndims = 50)
zx2_40ov <- FindNeighbors(zx2_40ov, dims = 1:30)
zx2_40ov <- FindClusters(zx2_40ov, resolution = 0.5)
...
```{r markerplots_test, fig.height=20, fig.width=20,}
# Markers for cluster identification
FeaturePlot(object = zx2_40ov, c("col1a1a", "cyp17a1", "fli1a", "lyz", "lck","mpeg1.1","gsdf", "lhx9","gpc1a","cx34.5","pcna","cxcl12a", "cara3","fgf24","en1b","tbx2b"), cols = c("grey90", "blue"), pt.size = 0.5)

```


```{r save_object_after_doublet_removal}
save(zx2_40ov_clean, file = "zx2_40ov_clean.robj")
